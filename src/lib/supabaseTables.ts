...
const mapProfessionalFromDb = (row: any): Professional => ({
  ...
  receivesCommission: row.receives_commission ?? true,
  commission: row.receives_commission === false ? 0 : Number(row.commission ?? 0),
...
export async function saveStateToSupabaseTables(state: State): Promise<void> {
  ...
  const professionals = state.professionals.map((p) =>
    clean({
      ...
      receives_commission: p.receivesCommission ?? true,
      commission: p.receivesCommission === false ? 0 : p.commission,
      color: p.color,
      active: p.active,
      updated_at: new Date().toISOString(),
    }),
